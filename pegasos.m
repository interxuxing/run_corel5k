function [wT,b] = pegasos(X,Y,lamda,kp,maxIter,Tolerance)
% Usage: [W,b] = pegasos(X,Y,lamda,kp,maxIter)
% Input:
...
% lamda, kp: parameters in Pegasos algorithm (default: lamda=1, k=0.1*n);
...
if(nargin<5 || isempty(kp)),    k=ceil(0.1*N);  else k = ceil(kp*N);  end
...
for t=1:maxIter
    ...
    At=X(idx,:);
    yt=Y(idx,:);
    Hingeloss = (At*w(t,:)'+b).*yt; 
    idx1= Hingeloss < 1;
%     fprintf('constrains %d in total %d pairs \n',length(find(idx1 == 1)), length(idx1));
    etat=1/(lamda*t);
    ...
    f(t+1) = lamda * sum(w(t+1,:).^2,2)/2 + sum(1 - Hingeloss(idx1),1)/k;
    if(mod(t, 100) == 0)
        fprintf('Iteration %d, lost function value %d \n',t, f(t+1));
    end
end
if(t<maxIter)
